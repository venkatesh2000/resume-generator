...
app.post("/information/deleteDetails", (req, res) => {
  UserInformationSchema.findOneAndDelete({ userId: req.body.pathName })
    .then((deletedDetails) => {
      if (deletedDetails)
        res.status(200).send("Deleted resume details from the database successfully!!")
      else
        res.status(200).send("Nothing to delete in the database!!")
    })
    .catch((_) => {
      res.status(500).send("Sorry, unable to delete resume details from the database!!")
    })
})
